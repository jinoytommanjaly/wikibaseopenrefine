...
import re
q_re = re.compile(r'(<?https?://opendatakerala.wikibase.cloud/(entity/|wiki/Item:))?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?https?://opendatakerala.wikibase.cloud/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
service_name = 'Reconcile for OpenDataKerala.'
...
SELECT ?child WHERE { ?child wdt:P2/wdt:P11* wd:$qid }
"""
...
SERVICE gas:service {
    ...
    gas:program gas:linkType wdt:P11 .
}
